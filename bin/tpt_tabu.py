...
from bin.utility import CalculateDistance
file_list = []
...
def init(_nodes):
    ...
    for i in range(len(_nodes)- 1):
        tempDist =[-1]
        prof.append(_nodes[i][2])
        for j in range(i+1, len(_nodes)):
            dist =\
            CalculateDistance(_nodes[i][0],_nodes[i][1],_nodes[j][0],_nodes[j][1])
...
def load_data(_file):
    ...
    for i in range(size):
        for j in range(i+1,size):
            distances.append(CalculateDistance(_nodes[i][0],_nodes[i][1],_nodes[j][0],_nodes[j][1]))
    print (_nodes)
    print ("\n Dinstances \n")
    print ( distances)
...
def parse():
    parser = argparse.ArgumentParser(description='Implementation of Tabu search')
